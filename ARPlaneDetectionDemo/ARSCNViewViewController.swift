    ...
    func renderer(_ renderer: SCNSceneRenderer, didAdd node: SCNNode, for anchor: ARAnchor) {
        if anchor.isMember(of: ARPlaneAnchor.classForCoder()) {
            ...
            if let planeAnchor = anchor as? ARPlaneAnchor {
                let plane = SCNBox.init(width: CGFloat(planeAnchor.extent.x*0.3), height: 0, length: CGFloat(planeAnchor.extent.x*0.3), chamferRadius: 0)
                ...
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + time) {
                    ...
                    vaseNode?.position = SCNVector3.init(planeAnchor.center.x, 0, planeAnchor.center.z)
